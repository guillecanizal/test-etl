...
public class SolrQueriesDAO extends QueriesDAO {
    ...
    private static final String QUERY_LIST_FLIGHTS_BY_AIRPORT_SORTED_BY_TIME = "http://localhost:8983/solr/testetl.flights/select?q=origin_airport_id%3A#FLIGHT_NUM#&start=1&rows=#ROWS#&sort=dep_time+asc&rows=100&fl=fl_num%2Cdep_time&wt=json&indent=true";
    ...
    public String listFlightsByAirportSortedByTime(int flightNum, int rows) {

        String json = null;
        List<Flight> flights = null;
        HttpResponse<JsonNode> response = null;
        try {
            String query = QUERY_LIST_FLIGHTS_BY_AIRPORT_SORTED_BY_TIME.replaceAll("#FLIGHT_NUM#", Integer.toString(flightNum));
